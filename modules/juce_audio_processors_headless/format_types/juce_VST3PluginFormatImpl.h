...
public:
    ...
    */
    ...
    void setStateInformation (const void* data, int sizeInBytes) override
    {
        ...
        {
            VSTComSmartPtr stream { new MemoryStream ((void*) data, sizeInBytes), IncrementRef::no };

            if (stream == nullptr || holder->component == nullptr)
                return;

            holder->component->setState (stream.get());
            ...
            if (editController != nullptr)
            {
                ...
                stream->seek (0, IBStream::kIBSeekSet, &result);
                editController->setState (stream.get());
            }
        }
    }
    ...
    void getPresetRaw (MemoryBlock& block) const override
    {
        ...
        if (holder->component->getState (&componentStream) != kResultOk)
        {
            block.setSize (0);
            return;
        }

        const auto compressedSize = componentStream.getSize();
        const auto* compressedData = componentStream.getData();
        ...
        MemoryInputStream compressedStream (compressedData,
                                            static_cast<size_t> (compressedSize),
                                            false);
        MemoryOutputStream decompressedStream;
        GZIPDecompressorInputStream gzipStream (&compressedStream,
                                                true,
                                                GZIPDecompressorInputStream::gzipFormat);
        decompressedStream << gzipStream;
        block.replaceWith (decompressedStream.getData(),
                           decompressedStream.getDataSize());
    }
